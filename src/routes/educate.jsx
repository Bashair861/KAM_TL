  ...
  head: () => ({
    meta: [
      { title: "Education — Aether KAM" },
      {
        name: "description",
        content:
          "Advisory sessions, best-practice playbooks, and modern industry suggestions for KAMs.",
      },
    ],
  }),
      ...
      <header className="h-16 bg-card border-b flex items-center justify-between px-8 sticky top-0 z-10">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Advisory sessions, playbooks and modern industry reading
          </p>
        </div>
      ...
      <div className="p-8 max-w-6xl w-full mx-auto space-y-8">
        ...
        <div className="bg-card border rounded-xl overflow-hidden">
          ...
          <p className="px-6 pt-3 text-[11px] text-muted-foreground">
            Curated articles from KAM, AI, and enterprise-tech sources — use them to spark
            conversations and strengthen client relationships.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
            {industryArticles.map((a) => (
              <div
                key={a.id}
                className="border rounded-lg p-4 hover:border-accent/40 transition-colors flex flex-col"
              >
