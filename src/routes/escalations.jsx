...
export const Route = createFileRoute("/escalations")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Active client escalations with SLA timers, RCA, and action items.",
      },
    ],
  }),
  ...
  return (
    <div className="bg-card border rounded-lg p-4 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-2">
        <span
          className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${esc.priority === "P1" ? "bg-crit/10 text-crit" : "bg-warn/10 text-warn"}`}
        >
          {esc.priority}
        </span>
